export const heavensake = {
  hero: {
    title: 'HEAVENSAKE',
    description:
      'HEAVENSAKE is a luxury sake brand that embodies a unique combination of Japanese tradition and French artistry, helmed by the acclaimed French cellar master Régis Camus.',
    tags: ['Brand Design'],
  },
  credits: [
    { id: 'brand_design', name: 'Brand Design', title: 'Serhii Churilov' },
    {
      id: 'brand_strategy',
      name: 'Brand Strategy',
      title: 'Vasyl Artiushchenko',
    },
  ],
};

export const koenigsegg = {
  hero: {
    title: 'Koenigsegg',
    description:
      "Koenigsegg is a Swedish avante-garde megacar and innovative technology company. For this client, we developed a design concept for a promo website that immerses users in the core values of the world's first four-seat hypercar, Gemera.",
    tags: ['Web Design', 'Motion Design'],
  },
  credits: [
    { id: 'art_direction', name: 'Art Direction', title: 'Serhii Churilov' },
    { id: 'web_design', name: 'Web Design', title: 'Serhii Churilov' },
    { id: 'motion_design', name: 'Motion Design', title: 'Serhii Churilov' },
    { id: 'music', name: 'Music', title: 'Dmitry Tkach' },
    { id: 'sound_design', name: 'Sound Design', title: 'Dmitry Tkach' },
    { id: 'management', name: 'Management', title: 'Danylo Matvienko' },
  ],
};

export const skyrocket = {
  hero: {
    title: 'Skyrocket',
    description:
      'Skyrocket is the digital agency for ambitious brands to achieve breakthroughs. In collaboration with Skyrocket, we developed a new branding, promo website, and motion design.',
    tags: ['Brand Design', 'Web Design', 'Motion Design'],
  },
  credits: [
    { id: 'art_direction', name: 'Art Direction', title: 'Serhii Churilov' },
    { id: 'ui_design', name: 'UI Design', title: 'Serhii Churilov' },
    { id: 'brand_design', name: 'Brand Design', title: 'Serhii Churilov' },
    { id: 'motion_design', name: 'Motion Design', title: 'Serhii Churilov' },
    {
      id: 'brand_strategy',
      name: 'Brand Strategy',
      title: 'Vasyl Artiushchenko ',
    },
    { id: 'ux_design', name: 'UX Design', title: 'Skyrocket' },
  ],
};

export const angle2 = {
  hero: {
    title: 'Angle2 Agency',
    livesite: 'https://angle2.agency/lab/',
    description:
      'Angle2 Agency is an Ukraine-based  design agency, that creates complex interfaces and interactive promo-web sites. Together with Angle2, we developed a new brand identity and a promotional website.',
    tags: ['Brand Design', 'Web Design'],
  },
  credits: [
    { id: 'art_direction', name: 'Art Direction', title: 'Serhii Churilov' },
    { id: 'ux_ui_design', name: 'UX/UI Design', title: 'Serhii Churilov' },
    { id: 'brand_design', name: 'Brand Design', title: 'Serhii Churilov' },
    {
      id: 'creative_frontDev',
      name: 'Creative FrontDev',
      title: 'Dmytro Oborskyi',
    },
    {
      id: 'management',
      name: 'Management',
      title: 'ANGLE2',
    },
  ],
};
